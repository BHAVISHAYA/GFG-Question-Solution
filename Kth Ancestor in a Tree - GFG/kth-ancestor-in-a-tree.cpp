...
void getParent(Node* root, int par, unordered_map<int, int>& parentMap) {
    if (root == nullptr) {
        return;
    }
    parentMap[root->data] = par;
    getParent(root->left, root->data, parentMap);
    getParent(root->right, root->data, parentMap);
}

int kthAncestor(Node* root, int k, int node) {
    unordered_map<int, int> parentMap;
    getParent(root, -1, parentMap);
    int nn = node;
